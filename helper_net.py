...
class Ops:
    ...
    def get_loss_pmask(X_pc, y_pmask_pred, Y_pmask):
        ...
        y_pmask_pred = tf.clip_by_value(y_pmask_pred, epsilon, 1. - epsilon)
        pmask_loss_focal_all = -Y_pmask*alpha*((1.-y_pmask_pred)**gamma)*tf.log(y_pmask_pred)\
                               -(1.-Y_pmask)*(1.-alpha)*(y_pmask_pred**gamma)*tf.log(1.-y_pmask_pred)
